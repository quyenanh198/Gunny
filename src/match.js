...
export class Match {
  constructor({ seed, character = "mochi", weapon = "carrot", difficulty = "normal", map = "sky" } = {}) {
    this.seed = seed;
    this.character = character;
    this.weapon = weapon;
    this.difficulty = DIFFICULTIES.find((d) => d.id === difficulty) || DIFFICULTIES[1];
    this.map = MAPS.find((entry) => entry.id === map) || MAPS[0];
    this.keys = new Set();
    this.reset();
  }
  reset() {
    ...
    this.actors = [
      { x: this.map.spawns[0], hp: START_HP, color: "#7ebbc9", name: player.name, skin: player.id, weapon: this.weapon, angle: 45 },
      { x: this.map.spawns[1], hp: START_HP, color: "#ec9b6c", name: bot.name, skin: bot.id, weapon: "acorn", angle: 135 },
    ];
