...
### 22. Ba việc: siết lại cách check khít-màn-hình, bỏ đổi súng giữa trận, sửa hố nhỏ hơn nhân vật

Ba phản hồi liên tiếp từ người dùng, đều xử lý:

**Check khít-màn-hình phải theo kích thước trình duyệt thật, không suy luận qua kích thước border.** `fits()` trong smoke test đã dùng `innerHeight` thật nhưng thiếu vế ngang; thêm `frame.left >= 0 && frame.right <= innerWidth + 1` để cả hai trục đều so trực tiếp với `window.innerWidth/innerHeight`, không qua bất kỳ phép đo gián tiếp nào.

**Đổi vũ khí giữa trận: đã hỏi lại người dùng, xác nhận cần bỏ** — character/map/súng là quyết định ở phòng chờ, khóa nguyên trận. Tìm thấy khối "VŨ KHÍ" trong màn trận (`.rack`/`#battleWeapons`) gọi thẳng `session.setWeapon()` mỗi lượt, và ở tầng server `case "loadout"` không hề chặn theo `this.state`, khác hẳn `team`/`ready`/`setup` — nghĩa là một client tự chế message vẫn đổi được vũ khí giữa trận dù không có UI. Sửa ở đúng tầng theo nguyên tắc "server là nguồn sự thật": thêm guard `if (this.state !== "lobby") return false` cho `case "loadout"`. Từ đó dọn toàn bộ đường dẫn giờ đã chết: `Match.setLoadout`, `RemoteMatch.setLoadout`, nhánh `state==="playing"` trong `OnlineSession`, `this.match?.setLoadout` trong `LocalSession`, khối tạo nút vũ khí trong `game.js`, đoạn đồng bộ `#battleWeapons` trong `hud.js`, và toàn bộ CSS `.rack`. Xóa 1 test đã hết ý nghĩa ("loadout changes are refused while charging or off turn"), sửa 1 test dựa vào hành vi cũ, viết lại test "changing weapon mid-match" thành khẳng định điều ngược lại: đổi vũ khí giữa trận không chạm vào trận đang chạy, chỉ ghi nhận cho trận sau.

**Hố nhỏ hơn nhân vật không nên làm rớt.** `resolveExplosion` lấy độ cao nền tại đúng 1 pixel dưới chân nhân vật (`terrain[Math.floor(actor.x)]`) rồi snap thẳng xuống — một hố hẹp trúng ngay điểm đó vẫn kéo nhân vật rớt dù hai bên chân còn nguyên đất. Thêm `FOOT_WIDTH=25` (khớp bán kính bóng đổ đã vẽ dưới chân trong `sprites.js`) và hàm `groundUnderFootprint`: quét toàn bộ bề rộng đứng, lấy điểm cao nhất còn nguyên vẹn làm nền — hố hẹp hơn `FOOT_WIDTH*2` thì đứng vững trên mép, hố rộng bằng hoặc hơn mới thật sự rớt. Đồng bộ luôn hằng số này vào bóng đổ trong `sprites.js` để hình vẽ và vùng va chạm khớp nhau, đúng như yêu cầu "tính lại border cho khớp animation".

Phát hiện phụ khi viết test cho footprint: `crater()` cho `NaN` khi bán kính không phải số nguyên, vì biên vòng lặp làm tỉ lệ `(i-x)/rx` vượt quá ±1 do làm tròn dấu phẩy động, khiến `sqrt` nhận số âm. Đây là lỗi **đang chạy thật trên production**: mọi phát S2 (`craterScale=1.1`) hay SS (`1.2`) đều nhân ra bán kính lẻ, tự làm hỏng một cột địa hình thành `NaN` vĩnh viễn ở mép hố mỗi lần bắn. Sửa bằng kẹp `Math.max(0, ...)` trước `sqrt`.

Xác minh: `npm run check` sạch, `npm test` 74/74 (thêm 4 test mới, xóa 1 test hết hiệu lực), smoke test browser thật chạy 3 lần liên tiếp không fail lần nào.
